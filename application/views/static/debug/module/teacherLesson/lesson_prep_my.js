...
define(function(require, exports){
    ...
    //检测文档是否转换好
    exports.inprocess = function(){
        $('.beike_file_trans_inprocess').live('click',function(){
            var file_id = $(this).attr('file_id');
            var url = baseUrlName + 'teacher/cloud/res/doc_process/' + file_id;
            var to_url = baseUrlName + 'teacher/lesson/file_detail/' + file_id;
            var that = $(this);
            $.tizi_ajax({
                'url' : url,
                'type' : 'GET',
    ...
    //动态查询 音视频 是否转换好
    exports.check_pfop = function(){
        $('.beike_check_pfop_file').live('click',function(){
            var file_id = $(this).attr('file_id');
    ...
    exports.myFileChangeFn = function(){
        $('.myFileChange').live('click',function(){
            ...
            //     }
            // });
            // $('.down_tip').html("提示：包含"+chosen+"个收藏文件，转存后将消耗"+chosen+"次本月备课文件下载数。");
            $.tiziDialog({
                id:'changeFile_id',
                title:'转存文件',
    ...
    exports.myFiledeleteFn = function(){
        $('.myFileDelete').live('click',function(){
            ...
            $.tiziDialog({
                ...
                ok:function(){
                    $.tizi_ajax({
                        'url' : baseUrlName+'lesson/lesson_prep_my/del_beike',
                        'type' : 'POST',
                        'dataType' : 'json',
                        'data' : {ids:id_str},
                        success : function(data){
...
/*  分页 相关function  start*/
    exports.init_ls_prep_my_page = {
        initPage:function(){
            if(typeof prep_min_page != 'function'){
                prep_min_page = function(page){
                    exports.page(page);                 
                }
            }
        }
    ...
    exports.get_files = function(getData){
        ...
        $.tizi_ajax({
            'url' : baseUrlName + 'lesson/lesson_prep_my/beike_list/'+getData['cat_id']+'/'+getData['sub_cat_id'],
            'type' : 'GET',
            'dataType' : 'json',
